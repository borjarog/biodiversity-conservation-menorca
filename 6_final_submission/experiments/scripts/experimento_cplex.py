...
def solve_conservation_cplex(data, budget=500, time_limit_sec=600, seed=0):
    print(f"\n⚡ [2/2] INICIANDO SOLVER CPLEX (Presupuesto: {budget} k€)")
    ...
    # Crear modelo CPLEX
    mdl = Model(name="Conservation_MIP")
    
    # Parámetros CPLEX
    mdl.parameters.timelimit = float(time_limit_sec)
    mdl.parameters.threads = 12
    mdl.parameters.mip.display = 0 # Silenciar log
    mdl.parameters.randomseed = seed
    # mdl.parameters.mip.tolerances.mipgap = 0.01 # Tolerancia 1% (opcional)
    ...
    for i in range(n_nodes):
        stress[i] = mdl.binary_var(name=f"stress_{i}")
        for s in range(n_species):
            x[i, s] = mdl.binary_var(name=f"x_{i}_{s}")
            y[i, s] = mdl.binary_var(name=f"y_{i}_{s}")
    ...
    z = {}
    for e in used_edges:
        z[e] = mdl.binary_var(name=f"z_{e}")

    # --- RESTRICCIONES Y OBJETIVO ---
    obj_terms = []
    cost_expr = 0
    active_area_terms = [0 for _ in range(n_species)]
    PENALTY_STRESS = int(350.0 * SCALE_SCORE)

    for i in range(n_nodes):
        # Stress: x[i,1] + x[i,2] <= 1
        mdl.add_constraint(x[i, 1] + x[i, 2] <= 1)
        # Stress penalty: stress >= x_martes + x_conejo - 1
        mdl.add_constraint(stress[i] >= x[i, 1] + x[i, 3] - 1)
        ...
        for s in range(n_species):
            ...
            if req_edges is None:
                mdl.add_constraint(x[i, s] == 0)
                continue

            # Inversión: y <= x
            mdl.add_constraint(y[i, s] <= x[i, s])
            
            # Si no es original, x <= y
            if not has_spec[s][i]: 
                mdl.add_constraint(x[i, s] <= y[i, s])
            
            # Coste Adaptación
            cost_expr += y[i, s] * cost_adapt[s][i]

            # Implicación de Ruta: x[i,s] <= z[e]
            if req_edges:
                for e in req_edges:
                    mdl.add_constraint(x[i, s] <= z[e])
    ...
    for e, var_z in z.items():
        ...
        obj_terms.append(var_z * -1)

    # Presupuesto
    if budget < 100_000: 
        mdl.add_constraint(cost_expr <= budget_int)

    # Equidad
    total_active = mdl.sum(active_area_terms)
    min_pct = [5, 20, 5, 15]
    
    for s in range(n_species):
        mdl.add_constraint(active_area_terms[s] * 100 >= min_pct[s] * total_active)
    ...
    # Resultados
    # Comprobar si se encontró solución (status óptimo o factible con límite de tiempo)
    # En CPLEX, solve_status puede ser JobSolveStatus.OPTIMAL_SOLUTION, FEASIBLE_SOLUTION, etc.
    # Usamos una comprobación simple de si hay solución
    if mdl.solution:
        ...
        status_str = str(mdl.get_solve_status()).split('.')[-1] # Simplificar string
        ...
        for i in range(n_nodes):
            ...
            for s_idx, s_name in enumerate(S_LIST):
                if paths_requirements[i][s_idx] is not None:
                    # CPLEX: .solution_value para obtener valor (redondear > 0.5)
                    row[f'active_{s_name}'] = 1 if x[i, s_idx].solution_value > 0.5 else 0
                    row[f'invest_{s_name}'] = 1 if y[i, s_idx].solution_value > 0.5 else 0
                else:
        ...
        print(f"❌ Fallo: No solution found ({total_time:.2f} s)")
        return None, None
...
if __name__ == "__main__":
    ...
    print(f"🚀 Iniciando análisis de robustez con CPLEX ({N_RUNS} ejecuciones)...")
    
    for i in range(N_RUNS):
        print(f"\n🔄 Ejecución {i+1}/{N_RUNS} (Seed={i})...")
        
        # IMPORTANTE: Llamar a la función de CPLEX
        df_sol, metrics = solve_conservation_cplex(datos, budget=PRESUPUESTO, seed=i)
        ...
        if metrics is not None:
            ...
            df_res.to_csv('metricas_robustez_cplex.csv', index=False)

    # 3. Análisis de Resultados
    df_res = pd.DataFrame(resultados_globales)
    
    print("\n" + "="*40)
    print("📊 RESUMEN ESTADÍSTICO (CPLEX - 30 RUNS)")
    print("="*40)
